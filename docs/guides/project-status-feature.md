...
1. Edit `.aios-core/core-config.yaml`:
   ```yaml
   projectStatus:
     enabled: true
...
projectStatus:
  ...
  statusFile: .aios/project-status.yaml  # Cache file location
...
- **Cache Location:** `.aios/project-status.yaml`
...
A: Yes, all agents use the same cache file (`.aios/project-status.yaml`).
